    ...
    for i in range(max_estimators):
        ...
        for j in range(max_features):
            ...
            for k in range(num_samples):
                ...
                forest = RandomForestClassifier(n_estimators = 10, max_features = 9) #See http://www.iro.umontreal.ca/~nie/IFT6255/class-email.pdf
                ...
                for a in range(total):
                    if forest.predict(vectorized_testing_data[a]) == testing_labels[a]:
                        correct += 1
                avg += (correct/total)
            if avg/num_samples > max_average[2]:
                max_average = [i, j, avg/num_samples]
